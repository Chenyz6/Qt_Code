...
namespace LDPCODEC {
	...
	*/
	...
	{
		read_gen(cfg->codec, cfg->gen_file, 0, 0);
	...
	{
		mod2dense *u = nullptr, *v = nullptr;
		double lk, maxlk, tpr;
		double *bpr = nullptr, *lk0 = nullptr, *lk1 = nullptr;
		char sblk[31];
		char *cblk = nullptr;
		unsigned d;
		int i, j;
	...
	*/
	...
	)
	{
		...
		initprp(cfg, H, lratio, dblk, bprb);
		...
		for (n = 0; ; n++)
		{
			...
			if (cfg->table == 2)
			{
				printf("%7d %5d %8.1f %6d %+9.2f %8.1f %+9.2f  %7.1f\n",
					cfg->block_no, n, changed(lratio, dblk, N), c, loglikelihood(lratio, dblk, N),
					expected_parity_errors(H, bprb), expected_loglikelihood(lratio, bprb, N),
					entropy(bprb, N));
			}

			if (n == cfg->max_iter || n == -cfg->max_iter || (cfg->max_iter>0 && c == 0))
			{
				break;
			}

			iterprp(cfg, H, lratio, dblk, bprb);
		}
